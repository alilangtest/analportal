...
@Service
@Transactional
public class ExcelOper_bak {
	
	private static final Logger logger = Logger.getLogger(ExcelOper_bak.class);
	//导入数据
		public static Map<String,String> exportData(String filePath,ExcelSubscribe excelSubscribe) 
			...
			if(listExcelSubscribe != null && listExcelSubscribe.size()>0){
				...
				for(int i=0;i<tabIdList.length;i++){
					...
					logger.info("查询数据条数的sql为："+sqlColumn);
					//查询出写入excel的数据总量
					...
					if(columnlList.size()>ByitConfig.EXP_EXCEL_MAXSIZE){
						...
						logger.info("查询并整理数据完成。当前时间"+Tools.getCurrFormatTimeGen()+",整理数据耗时："+(dataConfTime-confDataStartTime)+"ms");
...
//					logger.info("导出测试表信息成功！");
					logger.info("向磁盘中excel准备写入。当前时间"+Tools.getCurrFormatTimeGen()+",临时写入excel到现在花费时长："+(System.currentTimeMillis()-dealExcelStartTime));
					wbExcel.write(ouputStream);
					logger.info("写入excel完成。当前时间"+Tools.getCurrFormatTimeGen()+
								",写入excle耗时："+(System.currentTimeMillis()-dealExcelStartTime)+
