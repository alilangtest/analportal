...
$(".side-menu").on('click', 'li a', function(e) {
	...
	if(id!=""&&id!=null){
//		location.href = "dataAnalysis/dataAnalysis.html?id="+id;
		if(isIE()){
			location.href = "dataAnalysis.html?id="+id;
		}else{
			location.href = "dataAnalysis/dataAnalysis.html?id="+id;
		}
	}
	...
	for(var i=0; i<menu.length; i++){   
		...
		try{
			if(item.isHeader == "1"){
				...
				if(item.childMenus == ""){
					str = "<li><a href='"+item.url+"'><i class='icon-font icon-wenzhang'></i><span id="+item.id+">"+item.name+"</span></a></li>";
					$(parent).append(str);
				}else{
					str = "<li><a href='"+item.url+"'><i class='icon-font icon-caidan'></i><span id=''>"+item.name+"</span><i class='icon-font icon-icon-copy-copy'></i></a>";
