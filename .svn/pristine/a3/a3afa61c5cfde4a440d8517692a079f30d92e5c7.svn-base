...
public class ShiroDbAuthorizingRealm extends AuthorizingRealm {
	...
	Logger logger=Logger.getLogger(ShiroDbAuthorizingRealm.class);
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		...
		if(Contants.ISENABLE.equals(isEnable)){
			...
			//启用单点登录系统
			AuthUserEntity tUser=null;
			tUser=Utils.verifyLogin(token);
			logger.debug(Tools.getCurrFormatTimeGen()+"：sso服务端返回的用户为："+tUser);
			if(Tools.isNotEmpty(tUser.getUsername())){
				logger.debug(Tools.getCurrFormatTimeGen()+"：sso服务端返回的username为："+tUser.getUsername());
				user = authUserService.getEntityByUsername(tUser.getUsername());
				logger.debug(Tools.getCurrFormatTimeGen()+"：sso服务端返回的sessionid为："+tUser.getSessionId());
				user.setSessionId(tUser.getSessionId());
				logger.debug(Tools.getCurrFormatTimeGen()+"：sso服务端返回的tickst为："+tUser.getTicket());
				user.setTicket(tUser.getTicket());
			}
