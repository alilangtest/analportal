...
$(".side-menu").on(
		...
		function(e) {
			...
			if(!checkElement.parent("li").is('.menu-item')){
				...
				querylabel($this.find("span").eq(0).attr("id"));
			}
			...
			if (item.isHeader == "1") {
				...
				if (item.childMenus == "") {
					str = "<li><a href='" + item.url
							+ "'><i class='icon-font icon-wenzhang'>"
							// TODO
							+ "</i><span id='"+item.id+"'>" + item.name + "</span></a></li>";
					$(parent).append(str);
				} else {
					str = "<li class='menu-item'><a href='"
							+ item.url
							+ "'><i class='icon-font icon-caidan'>"
							+ "</i><span id='"+item.id+"'>"
							+ item.name
							+ "</span><i class='icon-font icon-icon-copy-copy'></i></a>";
...
$(function() {
	...
	var url = 'instantQuery/queryTree.do';
	$.post(url, function(result) {
		// var json = eval("(" + result.result+ ")");
		var menu = result.result;
	 ...
	 * initMenu(menu,$(".side-menu")); $(".side-menu >
				...
				.ajax({
					...
					success : function(data) {
						...
						for (var i = 0; i < labelAnalysis.length; i++) {
							...
							tableHTML1 += "<a href=\"javascript:void(0)\" onclick=\"edit("
									+ reportId
									+ ")\"><i class=\"fa fa-bar-chart-o\"></i>"
									+ labelAnalysis[i].reportName + "</a>";
							tableHTML1 += "</li>";
							tableHTML1 += "</ol>";
							tableHTML1 += "</li>";
						}
