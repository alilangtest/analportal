		...
		try {
			...
			if ("1".equals(report.getReportType().toString())) {
				...
				if (map != null) {
					...
					if(map.containsKey("orgName") && !"总行".equals(map.get("orgName").toString())){
						params += "&分行=" + map.get("orgName");
					}
				}
				else {
...
//					systemLogEntity.setMenuid("");
    ...
    @ResponseBody
	public List<TreeMap<String, Object>> getMenuTree(HttpServletRequest request){
		...
		//收藏  
		TreeMap<String, Object> hashmap1 = new TreeMap<String, Object>();
