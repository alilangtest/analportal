...
$(function() {
	...
	EL.hangnei = $('#hangnei');
	EL.hangwai = $('#hangwai');
	EL.hangnei.on('click', function() {
		$("#validitySpan").css("visibility","hidden");
	});
	EL.hangwai.on('click', function() {
		$("#validitySpan").css("visibility","visible");
	});
	//
	if ($.isEmpty(GV.id)) {
		EL.form.vals({});
