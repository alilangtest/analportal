        ...
        try {  
          ...
          MimeBodyPart mbpContent = new MimeBodyPart();  
            ...
            while (efile.hasMoreElements()) {  
                ...
                String subFileName = "";
                String OS_name=Tools.getOSName();
              //如果是windows系统，适用于开发环境
                if(OS_name.contains("Windows")){
                	logger.info("该操作系统为windows");
                	subFileName = filename.substring(filename.lastIndexOf("\\")+1,filename.length());
                	logger.info("附件名称为："+subFileName);
                }else if(OS_name.contains("Linux")){//如果是Linux系统，适用于生产环境
                	logger.info("该操作系统为linux");
                	subFileName = filename.substring(filename.lastIndexOf("/")+1,filename.length());
                	logger.info("附件名称为："+subFileName);
                }
                
                mbpFile.setFileName(MimeUtility.encodeText(subFileName));
