...
$(".side-menu").on('click', 'li a', function(e) {
	...
	if(checkElement.is('.menu-item-child')){
		checkElement.parent("li").parent("ul").parent("li").removeClass("active");
	}
	...
	for(var i=0; i<menu.length; i++){   
		...
		try{
			if(item.isHeader == "1"){
				...
				if(item.childMenus == ""){
					str = "<li><a href='"+item.url+"'><i class='icon-font icon-wenzhang'>"+"</i><span id="+item.id+">"+item.name+"</span></a>";
