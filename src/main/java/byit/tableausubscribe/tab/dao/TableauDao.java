...
public class TableauDao/* extends BaseDao*/{
	...
	public List<IdaPmReport> getViewsByReportInfo(String ids){
		...
		List<IdaPmReport> idsList =new ArrayList<IdaPmReport>();
		 Connection conn=null;
		 PreparedStatement pstmt=null;
		 ResultSet rs=null;
	...
	public Map<String,String> getTableauRefreshed(String reportids){
		...
		String refreshedTimeStr="";
		//Date refreshedTime=null;
		StringBuffer sql=new StringBuffer();
		sql.append(" SELECT to_char(extracts_refreshed_at,'yyyy-mm-dd hh24:mi') refreshed_time FROM workbooks WHERE 1=1 and refreshable_extracts ='t' and id=? ");
		 Connection conn=null;
		 PreparedStatement pstmt=null;
		 ResultSet rs=null;
		 try {
			 ...
			 for(String id : idsString){
				...
				rs = pstmt.executeQuery();
				while( rs.next()){
					//refreshedTime=rs.getDate("refreshed_time");
					refreshedTimeStr=rs.getString("refreshed_time");
					map.put(id, refreshedTimeStr);
				}
			 }
