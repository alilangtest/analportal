...
$(".side-menu").on('click', 'li a', function(e) {
	...
	if(!checkElement.parent("li").is('.menu-item')){
		$this.parent("li").parent("ul").parent("li").removeClass("active");
		$("li.active").removeClass('active');
		$this.parent("li").addClass("active");
		if($this.parent("li").parent("ul").is('.menu-item-child')){
			reportClass = $this.parent("li").parent("ul").prev().find("span").eq(0).html();
			reportSubclass = $this.find("span").eq(0).html();
			getPrev($this.parent("li").parent("ul").parent("li").prev(), queryInfo);
			//alert($this.parent("li.menu-header").eq(0).html() + "----" +$this.find("span").eq(0).html() + "---" + $this.parent("li").parent("ul").prev().find("span").eq(0).html() + "---" +$this.parent("li").parent("ul").parent("li").parent("ul").children().eq(0).html());
			//queryReportNames($this.parent("li").parent("ul").parent("li").parent("ul").children().eq(0).html(), $this.parent("li").parent("ul").prev().find("span").eq(0).html(),$this.find("span").eq(0).html());
		}
	}
	//防止有链接跳转
	e.preventDefault();
...
var reportSubclass;
function queryInfo(){
	queryReportNames(headerTrgt, reportClass,reportSubclass);
}
function getPrev(obj, cllBack){
	if(obj.is('.menu-header')){
		headerTrgt = obj.html();
		if(cllBack){
			cllBack();
		}
	}else{
		getPrev(obj.prev(), cllBack);
	}
}
/*添加iframe*/
	...
	for(var i=0; i<menu.length; i++){   
		...
		try{
			if(item.isHeader == "1"){
				...
				if(item.childMenus == ""){
					str = "<li><a href='"+item.url+"'><i class='icon-font icon-wenzhang'>"+"</i><span>"+item.name+"</span></a></li>";
					$(parent).append(str);
				}else{
					str = "<li class='menu-item'><a href='"+item.url+"'><i class='icon-font icon-caidan'>"+"</i><span>"+item.name+"</span><i class='icon-font icon-icon-copy-copy'></i></a>";
...
$(function(){
	//初始加载左侧菜单数据
	$.post(URLS.queryLeftData, function(result) {
		var json1 = eval("(" + result + ")");
		initMenu(json1,$(".side-menu"));
	});
	/*获取皮肤*/
	//getSkinByCookie();
